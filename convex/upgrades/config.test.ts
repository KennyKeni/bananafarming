...
test("DEFAULT_COST_MULTIPLIER is 2", () => {
  expect(DEFAULT_COST_MULTIPLIER).toBe(2);
});

test("click upgrade has 5x cost multiplier and low base cost", () => {
  const click = UPGRADES.find((u) => u.key === "click")!;
  expect(click.baseCost).toBe(5);
  expect(click.costMultiplier).toBe(5);
...
test("costAt doubles per owned with default multiplier", () => {
  expect(costAt(10, 1)).toBe(20);
  expect(costAt(10, 2)).toBe(40);
  expect(costAt(10, 3)).toBe(80);
  expect(costAt(10, 5)).toBe(320);
  expect(costAt(10, 10)).toBe(10240);
});
...
test("totalCps uses halved values for grandma/farm/mine", () => {
  expect(totalCps({ cursor: 3 })).toBe(3);
  expect(totalCps({ cursor: 2, grandma: 1 })).toBe(7);
  expect(
    totalCps({ cursor: 1, click: 10, grandma: 1, farm: 1, mine: 1, gstack: 1 }),
  ).toBe(1 + 5 + 50 + 500 + 5000);
});
